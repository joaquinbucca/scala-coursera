...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType =
    StructType(StructField(columnNames.head, StringType, nullable = false) :: columnNames.tail.map(c => StructField(c, DoubleType, nullable = false)))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val lists = columnNames.foldLeft(List.empty[(Int, Column)])((zero, name) => {
      activities.flatMap {
        case (prefixes, idx) if prefixes.exists(name.startsWith) => Some((idx, new Column(name)))
        case _ => None
      }.sortBy(_._1).headOption match  {
          case Some(pair) => pair :: zero
          case None => zero
      }
    }).groupBy(_._1).mapValues(_.map(_._2))

    (lists.getOrElse(0, List()), lists.getOrElse(1, List()), lists.getOrElse(2, List()))
  }
