...
object Huffman {
   ...
   *   }
   */
//  def times(chars: List[Char]): List[(Char, Int)] = (chars.map(c => (c, 1)) groupBy ((a, n) => (a, n + 1))).valuesIterator.toList
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case head :: second :: tail => makeCodeTree(head, second) :: tail
    case _ => trees
  }
  ...
  def until(sing: List[CodeTree] => Boolean, comb: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] =
    if (sing(trees) || trees.isEmpty) trees else until(sing, comb)(comb(trees))
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def search(t: CodeTree, ch: Char): List[Bit] = t match {
      case Leaf(_, _) => List()
      case Fork(l, r, _, _) => if (chars(l).contains(ch)) 0 :: search(l, ch) else 1 :: search(r, ch)
    }
