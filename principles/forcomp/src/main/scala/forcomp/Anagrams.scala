...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    case List() => List(List())
    case (ch, n) :: tail => (for {i <- 0 to n ; occ <- combinations(tail)} yield (ch,i) :: occ).toList map removeZeros
  }

  def removeZeros(list: Occurrences) : Occurrences = list.filter(_._2 != 0)
   ...
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = removeZeros(x.map(p => (p._1, p._2 - y.find(_._1 == p._1).getOrElse(("_", 0))._2 )))
